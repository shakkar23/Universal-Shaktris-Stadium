...
private:
    ...
    void parse_data(bool prev) {
        ...
        game.p1_game.current_piece = Piece((PieceType)p1.p_type, p1.p_rot, { p1.p_x, p1.p_y });
        game.p1_game.garbage_meter = p1.meter;
        game.p1_game.queue = { p1.queue[0], p1.queue[1], p1.queue[2], p1.queue[3], p1.queue[4] };
        game.p1_game.hold = p1.hold == 7 ? std::nullopt : std::optional(Piece((PieceType)p1.hold));

        game.p2_game.board = p2.b;
        game.p2_game.current_piece = Piece((PieceType)p2.p_type, p2.p_rot, { p2.p_x, p2.p_y });
        game.p2_game.garbage_meter = p2.meter;
        game.p2_game.queue = { p2.queue[0], p2.queue[1], p2.queue[2], p2.queue[3], p2.queue[4] };
